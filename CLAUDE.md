...
- `build-and-push` job: `needs: test`, and only on pushes to `main`. Builds the `Dockerfile` and pushes `ghcr.io/<owner>/ecommerce-api:latest` and `:sha-<sha>` using `GITHUB_TOKEN` with `packages: write`.
